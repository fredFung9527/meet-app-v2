...
import { fakeProducts } from '/data/note'
...
import ProductCardWithNote from './ProductCardWithNote'
  ...
  return (
    <div className="mx-2 mt-2">
      <Grid container spacing={2} alignItems="center">
        ...
        <Grid item xs={6} sm={3} md>
          <ProductListPicker value={filterSettings.productList} onChange={(v) => handleInput('productList', v)} optional hideHelperText/>
        </Grid>
        <Grid item xs={6} sm={3} md>
          <CheckBox
      ...
      <Grid container spacing={2} className="mt-2">
        {fakeProducts.map((product, idx) =>
          <Grid key={'product'+idx} item xs={6} sm={4} md={3}>
            <ProductCardWithNote {...product}/>
          </Grid>
        )}
      </Grid>
    </div>

...
export default function NewMeetingCustomerVisit(props) {
  ...
  React.useEffect(() => {
    ...
    const checkList = ['dateTime', 'marketCodeOrCompanyName', 'otherParticipants', 'location']
    let newErrors = {}
    let newHelperTexts = {}
      ...
      <Paper elevation={5} className="px-4 py-4">
        <Grid container spacing={2} alignContent="center">
          ...
          <Grid item xs={12} md={6}>
            <MeetingLocation
              required
              error={props.showError && errors['location']} 
              helperText={props.showError && errors['location'] && helperTexts['location']}
            />
          </Grid>
          <Grid item xs={12} md={6}>
